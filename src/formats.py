...
class AudioFile:
    def __init__(self, filename, size_out=1024, max_size=None):
        ...
        print(
            (
                "opening AudioFile %s with window size %i" % (filename, size_out)
    ...
    def read(self, sound_out=None):
        ...
        if sound_out is None:
            return self.size_out

        assert isinstance(sound_out, numpy.ndarray), (
            "sound_out must be a numpy.ndarray, but was was a %s" % sound_out.__class__
        )
        assert sound_out.dtype == numpy.complex64, (
            "sound_out must have dtype complex64, but had dtype %s" % sound_out.dtype
        )
        assert sound_out.shape == (
            self.size_out,
...
def write_image(image, filename, tol=1e-8, transpose=True, color=True):
    if color:
        return write_color_image(image, filename, tol, transpose)
    else:
        return write_gray_image(image, filename, tol, transpose)
