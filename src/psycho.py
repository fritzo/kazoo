...
def bark_transform (sampling_rate_hz):
  gamma = [ 1.0674,
            65.83,   # 1000x (R2)'s gamma_2, for Hz vs kHz
            0.1916 ]
  f = sampling_rate_hz
...
def mask_kernel (dw_bark):
  ...
  if   dw_bark < -1.3: return 0.0
  elif dw_bark < -0.5: return 10.0**(2.5 * (dw_bark + 0.5))
  elif dw_bark <  0.5: return 1.0
  elif dw_bark <  2.5: return 10.0**(-1.0 * (dw_bark -0.5))
  else:                return 0.0
...
def equalize (w_hz):
  ...
  return ( w2**2
         * (w2 + 5.68e7)
         / (w2 + 6.3e6)**2
         / (w2 + 3.8e8)
         / (w2**3 + 9.58e26) ) #optional term for cutoff above 5kHz
