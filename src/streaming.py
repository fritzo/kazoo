...
def Reals (*args, **kwds):
  ...
  if args: return network.Channel(_kazoo.Reals(*args), **kwds)
  else:    return network.Channel(allocator = _kazoo.Reals, **kwds)

def Complexes (*args, **kwds):
  "Factory for complex vector channels."
  if args: return network.Channel(transforms.Complexes(*args), **kwds)
  else:    return network.Channel(allocator = transforms.Complexes, **kwds)
...
class Audio (transforms.Audio):
  ...
  def start (self):
    ...
    if self.am_reading: self.start_read()
    if self.am_writing: self.start_write()

  def stop (self):
    if self.am_reading: self.stop_read()
    if self.am_writing: self.stop_write()
...
Screen      = network.Sink(transforms.Screen, 'vertical_sweep',
                                              'vertical_sweeping')
