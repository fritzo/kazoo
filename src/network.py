...
__all__ = [
    'stop_threads', 'Channel',
    'transform', 'source', 'sink', 'switched',
    'TwoWayStream', 'OneWayStream',
    ]
...
class Channel:
  ...
  # validation
  def __reading (self): self.__num_readers += 1
  def __writing (self): self.__num_writers += 1
  def __validate (self):
...
def source (fun):
  ...
  def new_fun (self, o):
    try:              o_shape = fun(self)
    except TypeError: o_shape = None
...
def sink (fun):
  ...
  def new_fun (self, i):
    try:              i_shape = fun(self)
    except TypeError: i_shape = None
