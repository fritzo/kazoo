...
def test1 ():
  ...
  def omega (t): return 2 * pi

  def delta (t): return 2 * t - 1
...
def test2b ():
  ...
  for i in range(1,N):
    t = (i + 0.5) / N
    dx,dy = time_deriv((X[i-1],Y[i-1]),
                       omega(t), theta(t), delta(t), nu(t))
    x = X[i-1] + dt * dx
    y = Y[i-1] + dt * dy
    r = sqrt(x**2 + y**2)
    X[i] = x / r
