...
class Concat2D:
    ...
    def __init__(self, *shapes_in):
        ...
        self.shape_out = (sum(x for x, y in shapes), max(y for x, y in shapes))
...
class Recorder:
    ...
    def write(self, data_in=None):
        ...
        assert isinstance(data_in, numpy.ndarray), (
            "data_in must be a numpy.ndarray, but was was a %s" % data_in.__class__
        )
        assert (
            data_in.dtype == self.data.dtype
        ), "data_in must have dtype %s, but had dtype %s" % (
            self.data.dtype,
            data_in.dtype,
        )
        assert data_in.shape == (
            self.size_in,
...
class ImageBuffer:
    ...
    def __init__(
        self, file_prefix, size_in, num_frames=None, color=True, filetype="jpg"
    ):
    ...
    def write(self, data_in=None):
        ...
        assert data_in.dtype == numpy.float32, (
            "data_in must have dtype numpy.float32, but had dtype %s" % data_in.dtype
        )
        assert data_in.shape == (
            self.size_in,
        ), "data_in must have shape (%i,) but had shape %s" % (
    ...
    def assemble(self):
        self.flush()
        formats.concat_images(self.__file_prefix, self.__filetype, width=self.__width)
...
def Bernstein_polynomials(number, size, sharpness=1.0, mapping=None):
    ...
    def binomial_coeff(n, m):
        return numpy.product(list(range(1 + m, 1 + n))) / numpy.product(
            list(range(1, 1 + n - m))
        )

    s = 1 - t
    basis = [
        binomial_coeff(number, n) * pow(t, n) * pow(s, number - n)
...
class Multigram:
  ...
  """
    ...
    def __init__(
        ...
        print(
            "building Multigram(%i, %i, %g)"
            % (small_exponent, large_exponent, sample_rate)
        )
        ...
        for size_exponent in range(small_exponent, 1 + large_exponent):
            ...
            supergram = Supergram(
                size_exponent, time_exponent, freq_exponent, self.sample_rate
            )
        ...
        for size in sizes:
            assert size == sizes[0], "supergram sizes disagree: %s != %s" % (
                size,
                sizes[0],
            )
        ...
        self.__coeffs = Bernstein_polynomials(
            len(self),
            self.size_out,
            sharpness=sharpness,
        ...
        if self.__single_thread:
            ...
            for s, t, d, x in zip(
                self.__switches, self.__supergrams, self.__delays, self.__super_parts
            ):
    ...
    # scale conversions
    def freq_scale(self, *args):
        return self.__supergrams[0].freq_scale(*args)

    freq_scale.__doc__ = Supergram.freq_scale.__doc__

    def pitch_scale(self, *args):
        return self.__supergrams[0].pitch_scale(*args)

    pitch_scale.__doc__ = Supergram.pitch_scale.__doc__

    # diagnostics
    def __len__(self):
        return len(self.__supergrams)
