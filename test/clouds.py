...
class ChiSquaredMLObjective:
    ...
    def __call__(self, d):
        ...
        return (
            (1 - d / 2) * (self.mean_log_x - log(2 * r2))
            + self.mean_x / (2 * r2)
            + log(2 * r2 * gamma(d / 2))
        )
...
def small_multi(dom_size=256, cod_size=128, grids=4):
    ...
    if not os.path.exists(gloves_file):
        gloves = subprocess.Popen(
            ["../src/learn", "gloves", "new"] + cod_grids + ["-o", gloves_file]
        )
        gloves.wait()
...
def gloves_multi(size=128, grids=5):
    ...
    gloves = subprocess.Popen(
        ["../src/learn", "gloves", "new"] + gloves_grids + ["-o", gloves_file]
    )
    gloves.wait()
...
def plot_chi2_objective(
    observed_mean=2.51, observed_mean_log=0.795, predicted_dof=33.9
):
