...
def get_stats(hash):
    ...
    if line:
        ...
        return (
            parts.get("files changed", parts.get("file changed", 0)),
...
def fourier_kde(data, weights=None, acuity=None, size=None):
    ...
    if acuity is None:
        acuity = sqrt(len(data))
    if size is None:
        size = round(10 * acuity)
...
def commitload():
    ...
    def to_hour(t):
        return t.tm_hour + (t.tm_min + t.tm_sec / 60.0) / 60.0

    def to_wday(t):
        return t.tm_wday + to_hour(t) / 24.0
    ...
    hour *= 24.0
    commits_hour /= total_hours
    wday *= 7.0
    commits_wday /= total_days
    ...
    ax.set_title(
        "Distribution of %i commits over %i days" % (len(times), int(total_days))
    )
...
def changeload():
    ...
    def to_week(t):
        return (t - epoch[0]) / (3600.0 * 24.0 * 7.0)
    ...
    week *= total_weeks
    changes_week /= total_weeks
    hour *= 24.0
    changes_hour /= total_hours
    wday *= 7.0
    changes_wday /= total_days

    def cmap(a):
        return (1 - 0.75 * a, 0.25 * a, 0.25 * a)
    ...
    # title
    ax.set_title(
        "Distribution of %i changes in %i commits over %i days"
...
def todo(max_items=32):
    ...
    for t in tuples:
        ...
        dated.append((int(date), fname, lineno, text.split("TO" + "DO")[-1].strip()))
