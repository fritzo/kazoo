...
def prob_to_energy (p): return log(1 / p - 1)
def energy_to_prob (e): return 1 / (1 + exp(e))
...
class Matching:
  ...
  def solve (self, num_iters, validate = True):
    for e in self.E: e.init(self.A[e])
    for v in self.V: v.init(self.F[v], self.M[v])

    if validate:
      for e in self.E: e.validate_problem()
      for v in self.V: v.validate_problem()

    for v in self.V: v.propagate()
    for i in range(num_iters):
      for e in self.E: e.propagate()
      for v in self.V: v.propagate()

    for e in self.E: e.constrain()
    for v in self.V: v.constrain()

    if validate:
      for e in self.E: e.validate_soln()
      for v in self.V: v.validate_soln()
