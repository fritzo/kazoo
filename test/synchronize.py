...
def beat_fun_circ(
    ...
    return (
        strength
        / radius
        * ((angle1 - radius) ** 2 + (angle2 - radius) ** 2 < radius**2)
    )
...
def beat_fun_pair_biquad_highpass(
    ...
    return (
        strength
        / radius
        * (beat_fun_biquad(angle1 / radius) - radius * 4 / 3 / (2 * pi))
        * beat_fun_biquad(angle2 / radius)
    )
...
def beat_fun_bicos_highpass(angle, radius=pitch_radius_biquad):
    return (
        maximum(0, cos(angle) - cos(radius))
...
def sim_strength(size=6, steps=40000, period=40):
    ...
    for t in range(steps):
        ...
        diff = sin(angle.reshape(1, size) - angle.reshape(size, 1)) * beat_fun_pair(
            angle.reshape(1, size), angle.reshape(size, 1)
        )
        force = sum(diff, 1)
...
def staircases(size=100, steps=10000, period=40):
    ...
    for p in param2:
        ...
        for t in range(steps):
            ...
            angle1 = 2 * pi * phase1
            angle2 = 2 * pi * phase2
            force = couple * sin(angle2 - angle1) * beat_fun_pair(angle2, angle1)

            dphase1 = freq1 * (1 + force)
            dphase2 = freq2 * (1 - force)

            phase1 += dphase1 / period
            phase1 -= floor(phase1)
            phase2 += dphase2 / period
            phase2 -= floor(phase2)
...
def lag(size=200, steps=40000, period=40):
    ...
    for t in range(steps):
        ...
        angle1 = 2 * pi * phase1
        angle2 = 2 * pi * phase2
        force = sin(angle2 - angle1) * beat_fun_pair(angle2, angle1)
...
def keys(size=100, octaves=1.0, steps=4000, period=40):
    ...
    for t in range(steps):
        ...
        angle1 = 2 * pi * phase1
        angle2 = 2 * pi * phase2
        force = sin(angle2 - angle1) * beat_fun_pair(angle2, angle1, radius)
...
def regions(size=100, steps=4000, period=40):
    ...
    for t in range(steps):
        ...
        for i in range(3):
            for j in range(i):
                force_ij = sin(angle[j, :, :] - angle[i, :, :]) * beat_fun_pair(
                    angle[j, :, :], angle[i, :, :]
                )
                force[i, :, :] += force_ij
                force[j, :, :] -= force_ij
